...
#
# Voraussetzung: restic installiert (`apt install restic` / `brew install restic`),
# RESTIC_REPOSITORY und RESTIC_PASSWORD in .env gesetzt.
#
# Einrichtung als tägliche Aufgabe (nicht automatisch von diesem Skript
# eingerichtet — bewusst, da Cron/systemd-Änderungen am System hier nicht
# selbständig vorgenommen werden sollen):
#   crontab -e
#   0 3 * * * cd /pfad/zu/Belegwirtschaft && ./scripts/backup.sh >> data/backup.log 2>&1
...
docker compose exec -T db pg_dump -U "${POSTGRES_USER}" "${POSTGRES_DB}" > data/db_dump.sql
